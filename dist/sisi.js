...
import { batchSize, loadModel, loadClip, } from './model.js';
import { buildIndex, removeInvalidIndex, getIndexPath, writeIndexToDisk, readIndexFromDisk, } from './indexing.js';
import { computeEmbeddingForQuery, } from './search.js';
/**
 * Build or update index for the dir.
 */
export async function index(targetDir) {
    // Read existing index.
...
export async function search(query, { maxResults, targetDir }) {
    ...
    for (let i = 0; i < Math.min(indices.size, maxResults); ++i) {
        ...
        const score = scores.index(index).item();
        if (score > goodScore) {
        ...
        results.push({
            filePath: images[index.item()].filePath,
            score: score * 100,
        });
    }
    return results;
 ...
 */
export async function removeIndex(targetDir) {
    targetDir = path.resolve(targetDir);
    const removed = [];
    const indexPath = getIndexPath();
    const index = await readIndexFromDisk(indexPath);
